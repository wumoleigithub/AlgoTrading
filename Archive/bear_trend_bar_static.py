...
def is_bear_trend_bar(idx, row, df):
    """
    Identify Bear Trend Bar based on given criteria.
    
    Args:
        idx (int): Index of the current row.
        row (pd.Series): The row data for the current bar.
        df (pd.DataFrame): The DataFrame containing the historical bars.
        
    Returns:
        bool: True if it's a Bear Trend Bar, False otherwise.
    """
    ...
    # Compute past LOOKBACK_BARS' average range
    avg_total_range = (df.iloc[idx-LOOKBACK_BARS:idx, df.columns.get_loc("high")] - df.iloc[idx-LOOKBACK_BARS:idx, df.columns.get_loc("low")]).mean()

    return row.close < row.open and body_ratio >= BODY_RATIO_THRESHOLD and total_range > avg_total_range

def apply_bear_trend_bar(df):
    """
    Apply the Bear Trend Bar detection function to the entire DataFrame.
    
    Args:
        df (pd.DataFrame): The DataFrame containing historical market data.
        
    Returns:
        pd.DataFrame: DataFrame with an added column 'BearTrendBar' (True/False).
    """
    df["BearTrendBar"] = [is_bear_trend_bar(i, row, df) for i, row in enumerate(df.itertuples())]
    return df
